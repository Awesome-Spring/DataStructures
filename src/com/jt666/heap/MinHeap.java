...
public class MinHeap<E extends Comparable<E>> {
    ...
    //上浮，如果该处元素小于父元素，跟父元素交换位置，继续上浮，直至到顶点
    ...
    /**
     * 下沉操作 判断要下沉元素是否右左孩子，如果右，则判断是否右右孩子，且左孩子和右孩子最小的是谁
     * 如果下沉元素小于左/右孩子 ，交换位置，继续下沉 直到没有左/右孩子 或要下沉的元素均小于左右孩子 下沉结束
     */
    private void siftDown(int k) {
        ...
        while (leftChild(k) < data.getSize()) {
            ...
            if (i + 1 < data.getSize() && data.get(i).compareTo(data.get(i + 1)) >0) {
                i++;//此时 data[i] 存储的是左右孩子中最小的
            }

            if (data.get(k).compareTo(data.get(i)) <= 0) {
                break;  //要下沉的节点均小于左右孩子 下沉结束
            }
            //否则，交换
            data.swap(k, i);
            k = i;//继续判断
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0 ; i < 1000 ; i ++) {
            minHeap.add( new Random().nextInt(Integer.MAX_VALUE));
        }
